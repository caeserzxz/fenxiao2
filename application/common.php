...
{
    //type=4 是有unionid，type=3是用的openid。
    $map = array();
    if ($type == 0)
    ...
    if ($type == 3 || $type == 4) {
        ...
        //$thirdUser = M('OauthUsers')->where($column,'like','%'.$user_id_or_name)->where('oauth',$oauth)->find();  //不用APP登录
        $thirdUser = M('OauthUsers')->where($column, 'like', '%' . $user_id_or_name)->find();    //用APP登录

        $map['user_id'] = $thirdUser['user_id'];
    }
    Log::alert(['user_id_or_name ' => json_encode($user_id_or_name)]);

    $user = M('users')->where($map)->find();
    Log::error('$map-users' . json_encode($user));
    return $user;
}
...
function queryExpress($shipping_code,$invoice_no)
{
    ...
    $post_data = array();
    $post_data["customer"] = 'F5035417204DC573771C918B8E128E0B';
    $key= 'DvDwxcyC4332' ;
//    $post_data["param"] = '{"com":"'.$invoice_no.'","num":"'.$shipping_code.'"}';
    $post_data["param"] = '{"com":"zhongtong","num":"75149673840234"}';
...
function kuaidiniao($ShipperCode,$LogisticCode,$RequestType=1002){

    $requestData= "{'OrderCode':'','ShipperCode':'$ShipperCode','LogisticCode':'$LogisticCode'}";

    $datas = array(
        'EBusinessID' => 1490972, //商户ID
        'RequestType' => $RequestType, //请求指令类型:1002-即时查询 1008-物流跟踪
        'RequestData' => urlencode($requestData) ,//数据内容签名
//        'DataType'    => 2, //请求、返回数据类型：2-json；
    );
    $datas['DataSign'] = encryptKDN($requestData, '580887f9-8c13-417a-b65e-b8f63c07e6d3'); //AppKey
 ...
 * @return DataSign签名
 */
function encryptKDN($data, $appkey) {
...
function userType($type){
    $userLevel = M('user_level')->where('level_id',$type)->getField('level_name');
    return $userLevel;
    switch ($type)
...
function update_pay_status($order_sn, $ext = array())
{
    ...
    if (stripos($order_sn, 'recharge') !== false) {
        ...
        // 如果这笔订单已经处理过了
        $count = M('order')->master()
            ->where("order_sn = :order_sn and pay_status = 0 OR pay_status = 2")
        ...
        if ($order['order_prom_type'] == 4) {
            ...
            if ($order['total_amount'] != $order['order_amount'] && $order['pay_status'] == 0) {
                ...
                \think\Hook::listen('oldPayAfter', $params);
...
//             }
        ...
        //分销设置（暂时关闭，已在AppPayment.php进行分佣）
 ...
 * @return bool|void
 */
function update_pay_status_old($order_sn, $ext = array())
{
    if (stripos($order_sn, 'recharge') !== false) {
        //用户在线充值
        $order = M('recharge')->where(['order_sn' => $order_sn, 'pay_status' => 0])->find();
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0)
{
    ...
    if ($pay_points < $min_use_limit_point) {
    ...
    $goods_arr = M('goods')->where("goods_id in(" . implode(',', $goods_id_arr) . ")")->cache(true, TPSHOP_CACHE_TIME)
        ->getField('goods_id,weight,market_price,is_free_shipping,exchange_integral,shop_price,is_free_shipping'); // 商品id 和重量对应的键值对
    $goods_weight = $goods_price = $cut_fee = $anum = $coupon_price = 0;  //定义一些变量

    /** @var int $total_integral 订单积分总价 */
    $total_integral = 0;
    foreach ($order_goods as $key => $val) {
        ...
        // 计算积分价格
        $order_goods[$key]['total_integral'] = $val['goods_num'] * $val['integral'];
        $total_integral += $order_goods[$key]['total_integral'];
        //计算商品邮费
//        if($goods_arr[$val['goods_id']]['is_free_shipping'] == 0){ //0-不包邮
//            $shipping_price += $goodsLogic->getPostage($province, $city, $district, $val['goods_num']);
//
//        }

    }

    $maxPoints = $result['order_integral'];

    // 计算该笔订单最多使用多少积分抵扣
    if (($use_percent_point != 1) && $pay_points > $maxPoints) {
        return array('status' => -1, 'msg' => "您使用的抵扣积分不能大于{$maxPoints}", 'result' => '积分'); // 返回结果状态
    }
    if (($pay_points > 0 && $use_percent_point == 0) || ($pay_points > 0 && $maxPoints == 0)) {
...
//    }
    ...
    $result = array(
        ...
        'total_integral' => $total_integral,// 商品积分总价
        'cut_fee' => $cut_fee, // 共节约多少钱
        'anum' => $anum, // 商品总共数量
        'min_use_points' => $min_use_limit_point,// 最大抵扣积分
        'max_use_points' => $maxPoints,// 最小抵扣积分
 ...
 * @return string
 */
function returnJson($status, $message = '', $data = array(), $pages = 1)
{
    $return = array();
