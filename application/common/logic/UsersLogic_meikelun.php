   ...
   */
    public function thirdLogin($data = array())
    {
        ...
        if ($data['first_leader']) {
            $_GET['first_leader'] = $data['first_leader'];
        }

        //Log::alert(['$openid || !$oauth' => $openid || !$oauth]);
        ...
        if (!$user) {
            ...
            //meikelun
            //注册前先判断是否有相同的unionid，有unionid一样，则是同一个用户。
            //unionid不存在，再判断openid，openid一样，则是同一个用户
            if ($unionid) {
                $oldUser = M('users')->where('unionid', $unionid)->find();
                if ($oldUser) {
                    //同一个unionid用户，直接登录
                    $user = $oldUser;

                    //登录送马克币
                    $makebiLogic=new MakebiLogic();
                    $makebiRt=$makebiLogic->login($user['user_id']);
                    ...
                    //新注册处理直推，管理关系，送马克币
                    $makebiLogic = new MakebiLogic();
                    $recommendId = '';    //扫码分享的推荐人id
                    $makebiRt = $makebiLogic->relation($row_id, $recommendId);
                ...
                //没有unionid，判断openid是否一致
                $oldUser = M('users')->where('openid', $openid)->find();
                if ($oldUser) {
                    //同一个openid用户，直接登录
                    $user = $oldUser;
            ...
            if (isset($data['first_leader'])) {
                $map['first_leader'] = $data['first_leader'];
            }
            $res = M('users')->where("user_id = '{$user['user_id']}'")->save($map);
            Log::alert(['更新成功返回信息res：' => $res]);
     ...
     * 第三方登录: (第一种方式:第三方账号直接创建账号, 不需要额外绑定账号)
     */
    public function thirdLogin2($data = array())
    {
        $openid = $data['openid']; //第三方返回唯一标识
        $oauth = $data['oauth']; //来源
        $unionid = $data['unionid']; //$unionid
     ...
     * @return array
     */
    public function reg($username, $password, $password2, $push_id = 0, $invite = array(), $user_id = "", $user_id2 = "", $nickname = "", $head_pic = "")
    {
        $is_validated = 0;
        ...
        //梅克伦注册送马克币
        $makebi_register = 0;
        $makebi_register_config = M('n_goods_config')->where('key', 'makebi_register')->find();
        $makebi_register = $makebi_register_config ? $makebi_register_config['value'] : 0;
        $obj = array();
        $obj['user_id'] = $user['user_id'];

        $regData = array();
        $regData['user_id'] = $user['user_id'];
        $regData['money'] = $makebi_register;
        $regData['reward_type'] = 0; //0不参与分销的奖励；1分销奖，2管理奖，3上荐奖
        $regData['type'] = 1;    //0人民币，1马克币，2功德，3佣金
        $regData['number'] = rand(10000, 99999) . time();
        $regData['desc'] = '注册获取马克币';
        $regData['obj'] = json_encode($obj);
        $regData['create_time'] = time();

        $Rt = M('n_amount_log')->add($regData);

        //记录用户马克币变动
        $users = M('users')->where('user_id', $user['user_id'])->find();
        $userUpdate = array();
        $userUpdate['total_makebi'] = $users['total_makebi'] + $regData['money'];
        $userUpdate['makebi'] = $users['makebi'] + $regData['money'];
...
//        vendor("phpqrcode.phpqrcode");
//        $QRcode = new \QRcode();
//        $QRcode->png($content,$outfile,$level,$size,$margin,$save);
//    }
    ...
    public function management($pid)
    {
        //通过session获取用户Id
