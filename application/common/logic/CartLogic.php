...
class CartLogic extends Model
{
    ...
    public function buyNow()
    {
        ...
        $buyGoods = [
            ...
            'integral' => $this->goods['integral'],// 积分价格
        ];
        ...
        //如果有阶梯价格,就是用阶梯价格
        if ($this->goods['price_ladder']) {
            $goodsLogic = new GoodsLogic();
            $price_ladder = unserialize($this->goods['price_ladder']);
            $buyGoods['member_goods_price'] = $goodsLogic->getGoodsPriceByLadder($this->goodsBuyNum, $this->goods['shop_price'], $price_ladder);
        }
        ...
        $buyGoods['total_integral'] = $cart->getTotalIntegralAttr(0, $buyGoods);
    ...
    /**
     * modify ：addGoodsExchange  生成兑换商品记录
     *
     * @return array
     * @throws \think\exception\DbException
     */
    public function addGoodsExchange()
    {

        if (empty($this->goods)) {
            return ['status' => -3, 'msg' => '兑换商品不存在', 'result' => ''];
        }
     ...
     * @throws \think\exception\DbException
     */
    private function addNormalApply()
    {
        if (empty($this->specGoodsPrice)) {
        ...
        if (!$this->user_id) {
            ...
            */
            $userCartGoods=M('n_apply_exchange')
        ...
        // 如果该商品已经存在兑换表，则不能再兑换
        if ($userCartGoods) {
            return array('status' => -4, 'msg' => '不能重复兑换', 'result' => '');
        } else {
            //如果该商品没有存在兑换记录
            if ($this->goodsBuyNum > $store_count) {
            ...
            $cartResult =M('n_apply_exchange')->insertGetId($cartAddData);
        }
        if ($cartResult !== false) {
            return array('status' => 1, 'msg' => '兑换成功', 'result' => '');
        } else {
            return array('status' => -1, 'msg' => '兑换失败', 'result' => '');
        }
    }
    ...
    public function AsyncUpdateCart($cart = [])
    {
        ...
        if ($cartList) {
            $cartList = collection($cartList)->append(['cut_fee', 'total_fee', 'goods_fee', 'total_integral'])->toArray();
     ...
     * @return array
     */
    public function getCartPriceInfo($cartList = null, $rate_type = null)
    {

        $total_fee = $goods_fee = $goods_num = $total_integral = 0;//初始化数据。商品总额/节约金额/商品总共数量

        if ($cartList) {
            foreach ($cartList as $cartKey => $cartItem) {
                $total_fee += $cartItem['goods_fee'];           //购物车的商品总价格
                $goods_fee += $cartItem['cut_fee'];
                $goods_num += $cartItem['goods_num'];           //购物车的商品总数量
                $total_integral += $cartItem['total_integral'];
            }
        }

        return compact('total_fee', 'goods_fee', 'goods_num', 'total_integral');
    }
