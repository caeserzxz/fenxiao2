...
class GoodsLogic extends Model
{
    ...
    {
        if (!$user_id) return array('status' => -1, 'msg' => '必须登录后才能收藏', 'result' => array());
     ...
     * @return int
     */
    public function getPostage( $province, $city, $district, $goods_num)
    {

        if ($goods_num == 0) return 0; // 商品没有重量

        // 先根据 镇 县 区找 shipping_area_id
        if ($district != 0) {
            $shipping_area_id = M('Region')->where("region_id = :region_id")->bind(['region_id'=>$district])->getField('postage');
        }

        // 先根据市区找 shipping_area_id
        if ($shipping_area_id == false && $city != 0) {
            $shipping_area_id = M('Region')->where("region_id = :region_id")->bind(['region_id'=>$city])->getField('postage');
        }

        // 市区找不到 根据省份找shipping_area_id
        if ($shipping_area_id == false && $province != 0) {
            $shipping_area_id = M('Region')->where("region_id = :region_id")->bind(['region_id'=>$province])->getField('postage');
        }

//        // 省份找不到 找默认配置全国的物流费
//        if ($shipping_area_id == false) {
//            // 如果市和省份都没查到, 就查询 tp_shipping_area 表 is_default = 1 的  表示全国的  select * from `tp_plugin`  select * from  `tp_shipping_area` select * from  `tp_area_region`
//            $shipping_area_id = M("ShippingArea")->where(['is_default'=>1])->getField('shipping_area_id');
//        }
        ...
        // 邮费= 商品数量 * 地区邮费
        $freight = $shipping_area_id['postage'] * $goods_num;
