...
class WechatUtil
{
    ...
    private function requestAndCheck($url, $method = 'GET', $fields = [])
    {
        ...
        Log::write('return :'.json_encode($return),'notice',true);
        if ($return === false) {
            $this->setError("http请求出错！");
            return false;
        }
        Log::write('测试 1','notice',true);
    ...
    {
        $wechat = $this->config;
        Log::write('wechat :'.json_encode($wechat),'notice',true);
        Log::write('测试token','notice',true);
        ...
        Log::write('微信appid :'.json_encode($appid),'notice',true);

        Log::write('微信appsecret :'.json_encode($appsecret),'notice',true);

        $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$appid}&secret={$appsecret}";
        $return = $this->requestAndCheck($url, 'GET');
        Log::write('returnToken :'.json_encode($return),'notice',true);
    ...
    public function getFanInfo($openid, $access_token=null)
    {
        if (null === $access_token) {
            ...
            Log::write('access_token :'.json_encode($access_token),'notice',true);
            if (!$access_token) {
                return false;
            }
        }
