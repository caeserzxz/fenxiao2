    ...
    public function index()
    {
        ...
        if ($res && $role) {
            foreach ($res as $val) {
                ...
                $provider = db('n_provider')->where('admin_id',$val['admin_id'])->find();
                $val['provider_name'] = $provider['name'];
...
//                $provider = db('n_provider')->where('id',$data['provider_id'])->find();
//                if(!empty($provider['admin_id'])){
//                    $this->error("该供应商已绑定对应管理员账户", U('Admin/Admin/admin_info'));
//                }

                $r = D('admin')->insertGetId($data);
//                $dataArr['admin_id'] = $r;
//                $updateProvider = db('n_provider')->where('id',$data['provider_id'])->update($dataArr);
            }
        }
...
//            $adminInfo = D('admin')->where('admin_id', $data['admin_id'])->find();
//            $dataArr['admin_id'] = null;
//            $updateProvider = db('n_provider')->where('id',$adminInfo['provider_id'])->update($dataArr);
...
//            dump($condition);die;
            if (!empty($condition['user_name']) && !empty($condition['password'])) {
                ...
                if (is_array($admin_info)) {
                    ...
                    if($adminInfo['type'] == 2){
//                        $this->redirect('user/teamList2');
                        $url = session('from_url') ? session('from_url') : U('Admin/user/teamList2');
                    }else{
    ...
    {
        $list = D('n_provider')->order('id desc')->select();
        $this->assign('list', $list);
        return $this->fetch();
    }
    ...
    public function add_provider()
    {
        ...
        $id = input('id');
        if($id){
        $providerInfo = db('n_provider')->where('id',$id)->find();
        }
    ...
    public function save_provider()
    {
        ...
        if(empty($id)){//添加操作
            if($data['password']!==$data['password2']){
                $this->error('操作失败',U('Admin/Admin/provider'));
            }
            $insert = db('')->insert($dataArr);
            if($insert){
                $this->success('添加成功',U('Admin/Admin/provider'));
            }else{
                $this->error('添加失败',U('Admin/Admin/provider'));
            }
        }else{//修改操作
            $update = db('n_provider')->where('id',$id)->update($dataArr);
            if($update !== false){
                $this->success('修改成功',U('Admin/Admin/provider'));
            }else{
                $this->error('修改失败',U('Admin/Admin/provider'));
            }
        }
    }
    ...
    public function del_provider()
    {
        $id = input('id');
        if(empty($id)){
            $this->error("删除失败",U('Admin/Admin/supplier'));
        }
        $del = db('n_provider')->where('id',$id)->delete();
        if($del){
            $this->success('修改成功',U('Admin/Admin/provider'));
