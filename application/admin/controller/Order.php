...
class Order extends Base
{
    ...
    public function ajaxjifen()
    {
        ...
        $sort_order = I('order_by', 'DESC') . ' ' . I('sort');
        //设置积分订单条件
        $condition['integral'] = array('neq', '0');
        $condition['order_amount'] = array('neq', '0');
        $count = M('order')->where($condition)->count();
        //dump($count);die;
        $Page = new AjaxPage($count, 20);
        $show = $Page->show();
    ...
    public function ajaxduishui()
    {
        ...
        //I('pay_status') != '' ? $condition['pay_status'] = I('pay_status') : false;
        //I('pay_code') != '' ? $condition['pay_code'] = I('pay_code') : false;
        I('shipping_status') != '' ? $condition['is_deliver'] = I('shipping_status') : false;   //发货状态
        I('user_id') ? $condition['user_id'] = trim(I('user_id')) : false;
        //$sort_order = I('order_by','DESC').' '.I('sort');
        $count = M('exchange_order')->where('is_delete', '<>', '1')->count();
        $Page = new AjaxPage($count, 20);
        $show = $Page->show();


        //获取订单列表
        $orderList = db('exchange_order')->where($condition)->where('is_delete', '<>', '1')->limit($Page->firstRow . ',' . $Page->listRows)->select();
        //dump($orderList);die;
        $list = array();
        foreach ($orderList as $key => $value) {
            $_t = $value;
            $users_data = db('users')->field('nickname,mobile,email')->where('user_id', $value['user_id'])->find();
            $_t['nickname'] = $users_data['nickname'];
            $_t['mobile'] = $users_data['mobile'];
            $_t['email'] = $users_data['email'];
            $list[] = $_t;
        }
    ...
    public function duishuidetail()
    {
        ...
        //查询兑水订单表
        $order_all = M('exchange_order')->where('id', $order_id)->find();
        $order_all['user_name'] = $input['user_name'];
        $order_all['mobile'] = $input['mobile'];
        $order_all['email'] = $input['email'];


        //根据address_id查询收货地址
        $address_data = M('user_address')->where('address_id', $order_all['address_id'])->find();
        $address = $orderLogic->getAddressName($address_data['province'], $address_data['city'], $address_data['district']);
        $address = $address . $address_data['address'];
        $address_data['detailed'] = $address;


        $goods_ids_array = explode(',', $order_all['goods_ids']);
        $list = array();
        foreach ($goods_ids_array as $key => $value) {
            //dump($value);die;
            //查询商品
            $Goods_data = M('goods')->where('goods_id', $value)->find();

            $Goods_data += M('exchange_split')->where('order_id', $order_all['id'])->where('goods_id', $value)->find();
            $list[] = $Goods_data;
        }
        //dump($list);die;
    ...
    //兑水删除表
    public function duishuidel()
    {
        //dump(input());die;
        $goods_id = M('exchange_split')->where('id', I('id'))->value('goods_id');
        // dump($goods_id);die;
        $res = M('exchange_split')->where('id', I('id'))->delete();

        $result = M('exchange_order')->where('id', I('order_id'))->value('goods_ids');
        $res1 = str_replace($goods_id, " ", $result);
        //dump($a);die;
        $result1 = M('exchange_order')->where('id', I('order_id'))->update(['goods_ids' => $res1]);
        return $res;
    }

    //兑水编辑
    public function duishuiedit()
    {
        ...
        $res = M('exchange_order')->where('id', $id)->find();
        if ($res['is_deliver'] == 1) {
            $this->error('已发货订单不允许编辑');
            exit;
        }
        ...
        //$orderGoods = $orderLogic->getOrderGoods($id);

        //根据兑水id查看 商品id
        $order_all = M('exchange_order')->where('id', $id)->find();

        $order_all['goods_ids'] = trim($order_all['goods_ids'], ' ,');
        //dump($order_all['goods_ids']);die;
        $goods_ids_array = explode(',', $order_all['goods_ids']);
        $list = array();
        foreach ($goods_ids_array as $key => $value) {
            //dump($value);die;
        ...
        if (IS_POST) {
            ...
            if (!$input['province'] || !$input['city'] || !$input['district'] || !$input['address']) {
                $this->error('收货地址不能为空');
            }
            ...
            //更新地址
            $update_dizhi = M('user_address')->where('address_id', $input['address_id'])
                ->update([
            ...
            //更新快递信息
            $update_wuliu = M('exchange_order')->where('id', $input['order_id'])->update(['shopping_type' => $input['shopping_type']]);
            ...
            if ($input['spec']) {
                ...
                for ($i = 0; $i < count($input['spec']); $i++) {
                    ...
                    $sum_money = $input['old_goods'][$i] * $input['per_goods_coin'][$i];

                    $res = M('exchange_split')->where('id', $a)->update(['num' => $input['old_goods'][$i], 'sum_coin' => $sum_money]);

                }
                //拆分表该商品的总价

                $zongjia = M('exchange_split')->where('order_id', $input['order_id'])->sum('sum_coin');

                //拆分表该订单id的总商品数
                $zongshu = M('exchange_split')->where('order_id', $input['order_id'])->sum('num');
                //更新订单表总价，总数
                $return = M('exchange_order')->where('id', $input['order_id'])->update(['total_coin' => $zongjia, 'goods_num' => $zongshu]);
        ...
        //根据address_id查询收货地址
        $address_data = M('user_address')->where('address_id', I('address_id'))->find();
        //dump($address_data);die;
        // 获取省份
        $province = M('region')->where(array('parent_id' => 0, 'level' => 1))->select();
        //获取订单城市
        $city = M('region')->where(array('parent_id' => $address_data['province'], 'level' => 2))->select();
        //获取订单地区
        $area = M('region')->where(array('parent_id' => $address_data['city'], 'level' => 3))->select();
        //获取支付方式
    ...
    */
            ...
            if ($data['refund_type'] == 0) {
                ...
                if ($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile') {
                    /*  header("Content-type: text/html; charset=utf-8");
                      exit("功能正在开发中。。。");*/
                    $updateOrder = array();
                    $updateOrder['pay_status'] = $data['pay_status'];
                    $updateOrder['refund_time'] = time();
                    $rt = M('order')->where('order_id', $data['order_id'])->update($updateOrder);
    ...
    public function detail($order_id)
    {
        ...
        if ($has_user) {
            $user = M("users")->field('user_id,nickname,id_card')->where(['user_id' => $order['user_id']])->find();
        }
        $user = M("users")->field('user_id,nickname,id_card')->where(['user_id' => $order['user_id']])->find();
    ...
    public function deliveryHandle()
    {
        ...
        $data['shipping_code'] = $data['invoice_no'];//单号
        $expressList = M('n_express')->where('id',$data['express_id'])->find();
        $data['invoice_no'] =$expressList['code'];
        $data['shipping_name'] = $expressList['name'];
        ...
        if ($res) {
            ...
            $this->success('操作失败', U('Admin/Order/delivery_list'));
        }
    }
    ...
    public function delivery_info()
    {
        ...
        if ($delivery_record) {
            //zxz
            $order['invoice_no'] = $delivery_record[count($delivery_record) - 1]['shipping_code'];
        }
        $this->assign('order', $order);
        $this->assign('orderGoods', $orderGoods);
...
//        var_dump($orderList);die;
     ...
     *
     * */
    public function export_ship()
    {
        //搜索条件
        $order_ids = I('order_ids');
        $where = array();//搜索条件
        $where['order_status']      = ['in', [0,1]]; //订单状态：0-待发货 1-待发货(后台确认订单) 2-待评价 3-已取消 4-已完成 5-已作废'
        $where['pay_status']        = 1; //支付状态：0-未支付 1-已支付;3-已同意退款；4-已拒绝退款
        $where['shipping_status']   = 0; //发货状态：0-未发货 1-已发货 2-部分发货
        if ($order_ids) {
            $where['order_id'] = ['in', $order_ids];
        }
...
//        echo json_encode($orderList);die;
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">快递公司</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">快递单号</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">卖家备注</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">收货人</td>';
...
//        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">支付状态</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">商品信息</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">商品数量</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="150">下单日期</td>';
        $strTable .= '</tr>';
        if (is_array($orderList)) {
            $region = get_region_list();
            foreach ($orderList as $k => $val) {
...
//                $strTable .= '<td style="text-align:left;font-size:12px;">' . $this->pay_status[$val['pay_status']] . '</td>';
    ...
    public function import($file = ''){
        ...
        $temp=file("$file");//连接EXCEL文件,格式为了.csv
...
//            }
    ...
    function daoru(){
        ...
        if(!empty($_FILES['inputExcel'])){
            ...
            if(!($ext == 'xls' || $ext == 'xlsx' || $ext == 'txt')){
                $this->error('文件格式不正确,请重新上传');
                return false;
            }
            $res = $this->uploadFile($filename,$tmp_name,$ext);
            return $res;
            if($res){
                return $res;
                //echo 'chengg';die;
     ...
     * $ext			文件后缀名
     **/
    function uploadFile($filename,$tmp_name,$ext){
        vendor('PHPExcel.PHPExcel');
        $filePath = './Public/uploadFile/';	//定义上传文件路径
        //判断是否有该路径,没有则创建
        if( !file_exists($filePath) ){
            mkdir($filePath);
        }
        //定义已上传的文件名
        $name = date('YmdHi', time()).'.'.$ext;
        //已上传的文件完整路径
        $uploadPath = $filePath.$name;
        //把文件移动到指定目录
        $result = move_uploaded_file($tmp_name, $uploadPath);
        ...
        if($result){
           ...
           /* $objPHPExcel = $objReader->load( $uploadPath ,$encode='utf-8');
            $objWorksheet = $objPHPExcel->getActiveSheet(0);//取第一张表

            $highestRow = $objWorksheet->getHighestRow();//总行数
            $highestColumn = $objWorksheet->getHighestColumn();//总列数(字母)
            $highestColumnIndex = \PHPExcel_Cell::columnIndexFromString($highestColumn);//总列数(数字)
...
//                }
            ...
            $objPHPExcel = $objReader->load($uploadPath,$encode='utf-8');//$file 为解读的excel文件
            //dump($objPHPExcel);die;
            $sheet = $objPHPExcel->getSheet(0);//取第一张表
            $highestRow = $sheet->getHighestRow(); // 取得总行数
            $highestColumn = $sheet->getHighestColumn();//总列数(字母)
            $highestColumnIndex = \PHPExcel_Cell::columnIndexFromString($highestColumn);//总列数(数字)
            ...
            {
                $order_num = $objPHPExcel->getActiveSheet()->getCell('A'.$j)->getValue();//订单编号
                ...
                $list[] =[$order_num,$poster,$logistics,$comm];
                //判断条件
                if(!is_null($order_num) && $order_num){
                    //查找订单
//                $orderInfo = M('order')->where('order_sn', $order_num)->find();
                    $res = M('order')->where('order_sn', $order_num)->find();
//                $res = $order_model->where('order_num',$order_num)->field('itemid,status')->find();
                    if ($res && ($res['order_status'] == 0 ||$res['order_status'] == 1)) {//判断订单状态是否待发货
...
//                        ],['itemid'=>$res['itemid']]);
                            //生成发货单，并更改订单发货状态
                            $data['order_id'] = $res['id'];//订单id
                            $data['order_sn'] = $order_num;//订单编号
                            $data['invoice_no'] = $poster;//物流单号
                            $data['user_id']    = $res['user_id'];//用户ID
                            $data['zipcode']    = $res['zipcode'];//邮编
                            $data['mobile']     = $res['mobile'];//联系电话
                            $data['consignee']  = $res['mobile'];//联系人
                            $data['country']    = $res['country'];//国家
                            $data['province']   = $res['province'];//省
                            $data['city']       = $res['city'];//市
                            $data['district']   = $res['district'];//区、县
                            $data['address']    = $res['address'];//地址
                            $data['shipping_price'] = $res['shipping_price']; //运费
                            $orderLogic = new OrderLogic();
                            $res = $orderLogic->deliveryHandle($data);
                            $success_item ++;
                        } else {
...
//            $data =array_pop($data);//移除第一行标题
            ...
            foreach ($data as $k=>$v){
                ...
                $logistics = $v[3];//物流单号
                $comm      = $v[4];//卖家备注
                $list2[] =[$order_num,$poster,$logistics,$comm];
...
//                $orderInfo = M('order')->where('order_sn', $order_num)->find();
                    ...
                    $orders[] = $res;
//                $res = $order_model->where('order_num',$order_num)->field('itemid,status')->find();
                  $expressInfo = M('n_express')->where('code', $poster)->find();
                    if ($res && ($res['order_status'] == 0) && ($res['pay_status'] == 1)) {//判断订单状态是否待发货
                        if ((!is_null($poster) && $poster) || (!is_null($logistics) && $logistics)) {
                            //更改状态
//                        OrderPlanModel::addLog($res['itemid'],'确认发货',$comm);
//                        $order_model->Update([
...
//                        ],['itemid'=>$res['itemid']]);
                            //生成发货单，并更改订单发货状态
                            $orderLogic = new OrderLogic();
                            $orderdata['goods']      = $orderLogic->getOrderGoods($res['order_id']);
                            $orderdata['shipping']   = 0;//发货状态：0-未发货  1-已发货  2-部分发货
                            $orderdata['express_id'] = $expressInfo['id'];//物流表id
                            $orderdata['order_id']   = $res['order_id'];//订单id
                            $orderdata['order_sn']   = $order_num;//订单编号
                            $orderdata['note']       = $comm;//发货备注
                            $orderdata['invoice_no'] = $poster;//物流单号
                            $orderdata['shipping_code'] = $logistics;//快递编号
                            $orderdata['shipping_name'] = $poster;//快递名称
//                            $orderdata['user_id']    = $res['user_id'];//用户ID
//                            $orderdata['zipcode']    = $res['zipcode'];//邮编
//                            $orderdata['mobile']     = $res['mobile'];//联系电话
//                            $orderdata['consignee']  = $res['mobile'];//联系人
//                            $orderdata['country']    = $res['country'];//国家
//                            $orderdata['province']   = $res['province'];//省
//                            $orderdata['city']       = $res['city'];//市
//                            $orderdata['district']   = $res['district'];//区、县
//                            $orderdata['address']    = $res['address'];//地址
//                            $orderdata['shipping_price'] = $res['shipping_price']; //运费
                            $datas2[] = $orderdata;
                            //处理发货单
                            $res = $orderLogic->deliveryHandle($orderdata);
                            if($res){
                                $success_item ++;
                            }else{
                                $fail_item ++;
...
//            return $orders;
//            return $datas2;
//            return $orderdata['express_id'] ;
//            return $orderdata['goods'] ;
            return $this->success('成功条数:'.$success_item.'，失败条数：'.$fail_item);
            return $list2;
        }
    }
     ...
     *
     * */
    public function import2($file = '',$tmp_name){
        $file['tmp_name'] ? $file['tmp_name'] : $file['tmp_name'] = $tmp_name;
        //设置获取excel对象
...
//                        ],['itemid'=>$res['itemid']]);
     ...
     * 分佣流水
     * */
    public function order_amount_log()
    {
        ...
        $count = M('n_rec_log')->where($where)->count();

        $page = new Page($count);
        $lists = M('n_rec_log')->where($where)
            ->order('id desc')
