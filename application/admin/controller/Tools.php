...
//                    array_map("unlink", glob($path));
    ...
    public function yunfei()
    {
        ...
        //香港仓运费
        if (isset($input['freight']) && $input['freight']) {
            $data['freight'] = $input['freight'];
        }

        //保税仓运费
        if (isset($input['b_freight']) && $input['b_freight']) {
            $data['b_freight'] = $input['b_freight'];
        }

        $res = db('region')->where('name', $input['name'])->update($data);
        if ($res) {
            return true;
        }
    }
