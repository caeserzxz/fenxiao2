...
//            'basic' => '基本设置',
    ...
    {
        //身份关系设置奖励列表
        $referralList = M('n_refferal')->where('id', '>', '0')->select();

        foreach ($referralList as $k => $v) {
            if ($v['sales_reward'] == '0') {
                $referralList[$k]['sales_reward'] = '/';
            }
            if ($v['management_reward'] == '0') {
                $referralList[$k]['management_reward'] = '/';
            }
            if ($v['top_reward'] == '0') {
                $referralList[$k]['top_reward'] = '/';
            }
        }
        $this->assign('referralList', $referralList);
        return $this->fetch();
    }
     ...
     * 保存身份等级奖励设置编辑
     * */
    public function referralSave()
    {
        $data = I('post.');

        if ($data) {
            foreach ($data['refer'] as $k => $v) {
                $referral = array();
                if ($v['sales_reward'] > '0') {
                    $referral['sales_reward'] = $v['sales_reward'];
                }
                if ($v['management_reward'] > '0') {
                    $referral['management_reward'] = $v['management_reward'];
                }
                if ($v['top_reward'] > '0') {
                    $referral['top_reward'] = $v['top_reward'];
                }

                $Rt = M('n_refferal')->where('id', $k)->update($referral);
            }
        }
   ...
   /*     $auto_confirm_date = tpCache('shopping.auto_confirm_date');
    ...
    function right_list()
    {
       /* $group = array('system' => '系统设置', 'content' => '内容管理', 'goods' => '商品中心', 'member' => '会员中心', 'finance' => '财务管理',
        ...
        $group = array('system' => '设置', 'users' => '会员','article' => '文章','notice' => '消息','admin' => '权限',
            'goods' => '商品','order' => '订单','apply' => '审核','advice' => '意见','wechat' => '微信');
    ...
    public function hkSet()
    {
        ...
        if ($input['add']) {
            ...
            foreach ($input['data'] as $k => $v) {
                ...
                //检测提现类型，只允许填写1或2
                $find = M('n_goods_config')->where('id',$k)->find();
                if(($find['key'] == "payment_type")){
                    if($v != 1 && $v != 2){
                        $this->error("提现类型只允许设置为1（百分比收取）或者2（固定收取）", U('Admin/System/hkSet'));
                    }
                }

                $Rt = M('n_goods_config')->where('id', $k)->update($data);
            }
            $this->success('保存成功', 'hkSet');
        } else {
            $data = M('n_goods_config')->where('is_show', '1')->select();
    ...
    public function explain(){
        $bizhong = M('n_explain')->where('map','explain')->find();
        if(IS_POST){
            $input = input('');
            $input['update_time'] = time();
            $update = M('n_explain')->where('map','explain')->update($input);
            if($update!==false){
                $this->success("更新成功", U('Admin/System/explain'));
            }else{
                $this->error("更新失败", U('Admin/System/explain'));
            }
        }
    ...
    //二维码背景图设置
    public function upQrCodeBackground()
    {
        $input = input();
        if(isset($input['add'])){
            $data['value'] = $input['img'];
            $retult = M('n_goods_config')->where('key','qecode_breakground')->update($data);
            if($retult!==false){
                $this->success("更新成功", U('Admin/System/upQrCodeBackground'));
            }else{
                $this->error("更新失败", U('Admin/System/upQrCodeBackground'));
            }
        }

        $img = M('n_goods_config')->where('key','qecode_breakground')->find();
