...
//    	$today = strtotime("-1 day");
        ...
        $count = array();
        //已到账佣金总和
        $count['countyj'] = Db::name('n_withdraw')
            ->where('status',2)
            ->sum('withdraw_money');
        //待返佣金总和
        $count['sumwaityj'] = Db::name('n_wait_commission')
            ->where('status',0)
            ->sum('money');
        //平台历史佣金总和
        $count['sumAllyj'] = Db::name('n_withdraw')
            ->where('status',2)
            ->sum('withdraw_money');
        //提现成功金额数
        $count['sumAllMoney'] = Db::name('n_withdraw')
            ->where('status',2)
            ->sum('withdraw_money');

        //提现未支付金额（审核中）   需要修改
        $count['sumUnCash'] = Db::name('n_withdraw')
            ->where('status',0)
            ->sum('withdraw_money');


        //会员总数（会员等级）
        $count['vip_total_user'] = M('users')
            ->where('user_type', '1')
            ->count();

        //非会员总数（不是会员等级）
        $count['no_vip_total_user'] = M('users')
            ->where('user_type', 'in', '0,2')
            ->count();

        //今天新增会员数（是会员等级）
        $count['today_vip_total_user'] = M('users')
            ->where('user_type', '1')
            ->where('reg_time', '>', $today)
            ->where('reg_time', '<', $today + 86400)
            ->count();

        //今天新增粉丝数（是粉丝等级）
        $count['today_fensi_total_user'] = M('users')
            ->where('user_type', '0')
            ->where('reg_time', '>', $today)
            ->where('reg_time', '<', $today + 86400)
            ->count();
