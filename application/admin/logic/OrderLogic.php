...
//        	$btn['delivery_confirm'] = '确认收货';
    ...
    public function deliveryHandle($data)
    {
        ...
        /*梅克伦把快递信息号存到order表*/
        $expressInfo = M('n_express')->where('id', $data['express_id'])->find();

        $orderData = array();
        $orderData['send_number'] = $data['shipping_code'];
        $orderData['shipping_code'] = $expressInfo['code'];
        $orderData['shipping_name'] = $expressInfo['name'];

        $orderDataRt = M('order')->where('order_id', $data['order_id'])->update($orderData);
        ...
        //梅克伦发货，发送站内消息
        $userNotice = array();
        $userNotice['user_id'] = $order['user_id'];
        $userNotice['content'] = '您的编号为:' . $order['order_sn'] . '的订单已发货，请注意收货。';
        $userNotice['create_time'] = time();
        $userNotice['type'] = 2;    //消息类型：1系统通知。2订单消息
        $userNotice['is_send'] = 1;    //是否发送，0未发送，1已发送
        $noticeRt = M('n_user_notice')->add($userNotice);
    ...
    function delOrder($order_id)
    {
        ...
        exit(json_encode(array('status' => 1, 'msg' => '删除成功', 'data' => array('url' => U('Admin/order/index')))));
    }
     ...
     * $data  要导出的数组
     */
    public function exportList($rows, $data, $List)
    {
        ...
        foreach ($rows as $k => $v) {
            $strTable .= '<td style="text-align:center;font-size:12px;width:120px;">' . $v . '</td>';
        }
        $strTable .= '</tr>';
        if (is_array($data)) {
            foreach ($List as $p => $q) {
                ...
                foreach ($data as $k => $val) {
                    $strTable .= '<td style="text-align:left;font-size:12px;">' . $List[$p][$val] . ' </td>';
                }
                $strTable .= '</tr>';
            }
        }
