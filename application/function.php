 ...
 */
function GetJsApiParameters($data,$key){
    $wxPay['appId'] = $data['appid'];
    $wxPay['timeStamp'] = time();
    $wxPay['nonceStr'] = getNonceStr();
    $wxPay['package'] = "prepay_id=" . $data['prepay_id'];
    $wxPay['signType'] = "MD5";
    $wxPay['paySign'] = MakeSign($wxPay,$key);
    return $wxPay;
}
 ...
 */
function MakeSign($parameter,$key)
{
    //签名步骤一：按字典序排序参数
    ksort($parameter);
    $string = ToUrlParams($parameter);
    //签名步骤二：在string后加入KEY
 ...
 * 格式化参数格式化成url参数
 */
function ToUrlParams($parameter)
{
    $buff = "";
...
//                       curl_setopt($ch,CURLOPT_CAINFO, getcwd().'/cert/apiclient_cert.PEM');
        ...
        curl_setopt($ch,CURLOPT_SSLCERT, getcwd().'/extend/wechat/payment/cacert/apiclient_cert.pem');
        curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
        curl_setopt($ch,CURLOPT_SSLKEY, getcwd().'/extend/wechat/payment/cacert/apiclient_key.pem');
    }
...
function checkReturn($data){
    ...
    while(1){
        ...
        if($sum >= 6){
            ...
            $output = $this->http_curl($this->getIp()."/invoke/order/webNotify?data=".$data);
