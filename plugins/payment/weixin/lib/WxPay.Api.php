...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsOut_trade_noSet()) {
            Log::error('wxPayApi$inputObj-11');
			throw new WxPayException("缺少统一支付接口必填参数out_trade_no！");
		}else if(!$inputObj->IsBodySet()){
            Log::error('wxPayApi$inputObj-22');
			throw new WxPayException("缺少统一支付接口必填参数body！");
		}else if(!$inputObj->IsTotal_feeSet()) {
            Log::error('wxPayApi$inputObj-33');
			throw new WxPayException("缺少统一支付接口必填参数total_fee！");
		}else if(!$inputObj->IsTrade_typeSet()) {
            Log::error('wxPayApi$inputObj-44');
			throw new WxPayException("缺少统一支付接口必填参数trade_type！");
		}
		
		//关联参数
		if($inputObj->GetTrade_type() == "JSAPI" && !$inputObj->IsOpenidSet()){
            Log::error('wxPayApi$inputObj-55');
			throw new WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");

		}
		if($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()){
            Log::error('wxPayApi$inputObj-66');
      ...
      /*  Log::error('appid'.WxPayConfig::$appid);
        Log::error('macid'.WxPayConfig::$mchid);
        Log::error($_SERVER['REMOTE_ADDR']);
        Log::error('SetNonce_str-'.self::getNonceStr());*/

		//签名
		$signName=$inputObj->SetSign();
